...
//      "<link rel=\"stylesheet\" href=\"breviar.css\">\n"
  ...
  if (coord) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "svpismo", "starting search %s\n", search);
    fulltext_search(search);
    __android_log_print(ANDROID_LOG_INFO, "svpismo", "search finished\n");

    while (get_fulltext_result(&b, &h, &v, &t)) {
      int i,in,var;
      __android_log_print(ANDROID_LOG_INFO, "svpismo", "printing item\n");
      snprintf(buf, sizeof(buf), "%s%d", b,h);
      tmp = StringEncode(buf);
    ...
    __android_log_print(ANDROID_LOG_INFO, "svpismo", "done\n");
    ...
    __android_log_print(ANDROID_LOG_INFO, "svpismo", "freeing...\n");
  ...
  {
    __android_log_print(ANDROID_LOG_INFO, "svpismo", "encoding...\n");
  ...
  __android_log_print(ANDROID_LOG_INFO, "svpismo", "free kontext\n");
  FreeBuf(&kontext);
  __android_log_print(ANDROID_LOG_INFO, "svpismo", "free out\n");
  FreeBuf(&out);

  __android_log_print(ANDROID_LOG_INFO, "svpismo", "db_close\n");
  db_close();
  __android_log_print(ANDROID_LOG_INFO, "svpismo", "return\n");
  return jout;
}
